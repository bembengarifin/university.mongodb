...
function PostsDAO(db) {
    ...
    this.insertEntry = function (title, body, tags, author, callback) {
        ...
        posts.insert(post, { w : 1}, function(err, result) {
        	...
        	console.log("new post was inserted, result:" + result);
    ...
    this.addComment = function(permalink, name, email, body, callback) {
        ...
        this.getPostByPermalink(permalink, function(err, post) {
        	...
        	//console.log(post);
        	posts.update({ '_id' : post._id }, { $set : { 'comments' : post.comments } }, { w : 1 }, function(err, recordsUpdated) {
        		if (err) return callback(err, null);
        		
        		callback(err, recordsUpdated);
        	});
        });
